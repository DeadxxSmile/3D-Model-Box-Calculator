...
def calc_box(box_values):
    ...
    try:
        box_results = {
            "box_width": (box_values["item_width"] + (box_values["item_slop"] * 2)) * box_values["item_count"] + (box_values["outer_wall"] * 2) + ((box_values["item_count"] - 1) * box_values["inner_wall"]),
...
def calc_window(box_settings, lid_settings, window_settings):
    ...
    try:
        window_results = {
            "window_inset": lid_settings["lid_wall"] + lid_settings["lid_gap"] + box_settings["outer_wall"] + window_settings["window_overhang"],
            "window_inner_wall": box_settings["inner_wall"] + (window_settings["window_separator"] * 2),
            "window_total_height": (box_settings["item_height"] + (box_settings["item_slop"] * 2)) - (window_settings["window_overhang"] * 2),
            "outer_window_width": (box_settings["item_width"] + (box_settings["item_slop"] * 2)) - window_settings["window_overhang"] - window_settings["window_separator"],
            "inner_window_width": (box_settings["item_width"] + (box_settings["item_slop"] * 2)) - (window_settings["window_separator"] * 2)
        }
        
        if box_settings["item_count"] == 1:
            total_width = (box_settings["item_width"] + (box_settings["item_slop"] * 2)) - (window_settings["window_overhang"] * 2)
        elif box_settings["item_count"] == 2:
            window_total_width = (window_results["outer_window_width"] * 2) + window_results["window_inner_wall"]
        elif box_settings["item_count"] == 3:
            window_total_width = (window_results["outer_window_width"] * 2) + window_results["inner_window_width"] + (window_results["window_inner_wall"] * 2)
        elif box_settings["item_count"] > 3:
            window_total_width = (window_results["outer_window_width"] * 2) + (window_results["inner_window_width"] * (box_settings["item_count"] - 2))+ (window_results["window_inner_wall"] * (box_settings["item_count"] - 1))
        else:
